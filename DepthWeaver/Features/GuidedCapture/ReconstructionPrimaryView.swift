...
struct ReconstructionPrimaryView: View {
    @EnvironmentObject var appModel: GuidedCaptureModel
    let outputFile: URL
...
struct ReconstructionProgressView: View {
    ...
    var body: some View {
        ...
        .task {
            // SwiftUI may re-evaluate this view (and re-fire .task) after reconstruction
            // has already moved appModel into .viewing — for example when an outer sheet
            // such as NameCaptureSheet dismisses or when the host view pops. In that case
            // photogrammetrySession is already nil and there's nothing to drive.
            guard appModel.state == .reconstructing,
                  let session = appModel.photogrammetrySession else {
                return
            }
    ...
    struct LocalizedString {
        static let cancel = NSLocalizedString(
            ...
            bundle: GuidedCaptureModel.bundleForLocalizedStrings,
            value: "Cancel",
