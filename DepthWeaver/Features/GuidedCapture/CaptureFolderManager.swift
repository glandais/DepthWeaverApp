...
class CaptureFolderManager: ObservableObject {
    ...
    // plus the final model-mobile.usdz. Created in the system temporary directory so
    // abandoned scans are reclaimed by iOS automatically.
    let rootScanFolder: URL

    // Subdirectory of `rootScanFolder` for images (required by ObjectCaptureSession).
    let imagesFolder: URL

    // Subdirectory of `rootScanFolder` for checkpoints
    // (required by both ObjectCaptureSession and PhotogrammetrySession).
    ...
    /// Creates a new Scans directory based on the current timestamp inside the system
    /// temporary directory.
    /// - Returns: The new Scans folder's file URL, or `nil` on error.
    static func createNewScanDirectory() -> URL? {
        let capturesFolder = rootScansFolder()
    ...
    /// Returns the temporary folder used as the parent of all in-progress scans.
    /// iOS reclaims this directory automatically, so abandoned scans don't accumulate.
