...
@MainActor
@available(iOS 17.0, *)
class GuidedCaptureModel: ObservableObject, Identifiable {
    let logger = Logger(subsystem: "io.github.glandais.depthweaver",
                                category: "GuidedCaptureModel")

    static let instance = GuidedCaptureModel()
    ...
    @Published var state: ModelState = .notSet {
        didSet {
            logger.debug("didSet GuidedCaptureModel.state to \(self.state)")
    ...
    func determineCurrentOnboardingState() -> OnboardingState? {
        ...
        if session.numberOfShotsTaken >= GuidedCaptureModel.minNumImages {
            switch orbit {
                case .orbit1:
