...
enum Model3DDepthRenderer {
    ...
    ///
    /// One-shot convenience: builds a throwaway ``Session``. Callers that
    /// capture repeatedly — live mode captures on every gesture frame — should
    /// hold on to a `Session` instead, so the Metal textures and the
    /// `SCNRenderer` are allocated once.
    static func captureDepthMap(from scnView: SCNView, outputSize: CGSize) -> DepthMap? {
        ...
        guard let pov = scnView.pointOfView else {
            logger.error("captureDepthMap: scnView has no pointOfView")
            return nil
        }
        guard let session = Session(device: scnView.device) else { return nil }
        return session.captureDepthMap(scene: scene, pointOfView: pov, outputSize: outputSize)
    }

    /// A reusable offscreen depth-capture pipeline.
    ///
    ...
    /// when computing the auto range, then pushed behind the subject.
    final class Session {
        ...
        private var stagingColorTex: MTLTexture?
        private var depthTex: MTLTexture?
        private var stagingDepthTex: MTLTexture?
        private var textureWidth = 0
        private var textureHeight = 0

        init?(device: MTLDevice? = nil) {
            guard let dev = device ?? MTLCreateSystemDefaultDevice() else {
                logger.error("Session: no Metal device")
                return nil
            }
            guard let queue = dev.makeCommandQueue() else {
                logger.error("Session: failed to create command queue")
                return nil
            }
            self.device = dev
            self.commandQueue = queue
            self.renderer = SCNRenderer(device: dev, options: nil)
        }

        func captureDepthMap(scene: SCNScene, pointOfView: SCNNode, outputSize: CGSize) -> DepthMap? {
            ...
            guard let camera = pointOfView.camera else {
                logger.error("captureDepthMap: pointOfView has no camera")
                return nil
            }
            guard allocateTexturesIfNeeded(width: width, height: height),
                  let colorTex, let stagingColorTex, let depthTex, let stagingDepthTex
            else {
                return nil
            }
            ...
            logger.debug("captureDepthMap: zNear=\(near) zFar=\(far) fgPixels=\(fgCount) ndcRange=\(fgNdcMin)...\(fgNdcMax) reversedZ=\(isReversedZ)")
        ...
        /// Render targets are kept between captures: live mode asks for the same
        /// size dozens of times a second, and reallocating four textures each
        /// time is the one part of this pipeline that would show up as jitter.
        private func allocateTexturesIfNeeded(width: Int, height: Int) -> Bool {
            if width == textureWidth, height == textureHeight,
               colorTex != nil, stagingColorTex != nil, depthTex != nil, stagingDepthTex != nil {
                return true
            }

            func makeTexture(pixelFormat: MTLPixelFormat, renderTarget: Bool) -> MTLTexture? {
                ...
                desc.usage = renderTarget ? [.renderTarget, .shaderRead] : [.shaderRead]
                desc.storageMode = renderTarget ? .private : .shared
                return device.makeTexture(descriptor: desc)
            }

            guard let color = makeTexture(pixelFormat: .bgra8Unorm, renderTarget: true),
                  let colorStaging = makeTexture(pixelFormat: .bgra8Unorm, renderTarget: false),
                  let depth = makeTexture(pixelFormat: .depth32Float, renderTarget: true),
                  let depthStaging = makeTexture(pixelFormat: .depth32Float, renderTarget: false)
            else {
                logger.error("captureDepthMap: failed to allocate render targets (\(width)x\(height))")
                colorTex = nil
                stagingColorTex = nil
                depthTex = nil
                stagingDepthTex = nil
            ...
            colorTex = color
            stagingColorTex = colorStaging
            depthTex = depth
            stagingDepthTex = depthStaging
            textureWidth = width
            textureHeight = height
            return true
        }

        private func makeFloat32PixelBuffer(width: Int, height: Int, values: [Float]) -> CVPixelBuffer? {
